...
class GatedConv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride = 1, padding = 0, dilation = 1, pad_type = 'reflect', activation = 'lrelu', norm = 'none', sn = False):
        super(GatedConv2dLayer, self).__init__()
        # Initialize the padding scheme
        if pad_type == 'reflect':
            self.pad = nn.ReflectionPad2d(padding)
        elif pad_type == 'replicate':
...
class TransposeGatedConv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride = 1, padding = 0, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = True, scale_factor = 2):
        super(TransposeGatedConv2dLayer, self).__init__()
        # Initialize the conv scheme
        self.scale_factor = scale_factor
...
class ResGatedConv2dLayer(nn.Module):
    def __init__(self, channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = True):
        super(ResGatedConv2dLayer, self).__init__()
        # Initialize the conv scheme
        self.conv2d = nn.Sequential(
            GatedConv2dLayer(in_channels, out_channels, kernel_size, stride, padding, dilation, pad_type, activation, norm, sn),
            GatedConv2dLayer(out_channels, out_channels, kernel_size, stride, padding, dilation, pad_type, activation = 'none', norm = norm, sn = sn)
        )
...
#               Dense Block
# ----------------------------------------
class DenseConv2dLayer_5C(nn.Module): # DB
    def __init__(self, channels, latent_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = True):
        super(DenseConv2dLayer_5C, self).__init__()
        self.conv1 = Conv2dLayer(channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv2 = Conv2dLayer(channels + latent_channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv3 = Conv2dLayer(channels + latent_channels * 2, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv4 = Conv2dLayer(channels + latent_channels * 3, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv5 = Conv2dLayer(channels + latent_channels * 4, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
...
class ResDenseConv2dLayer_5C(nn.Module): # RDB
    def __init__(self, channels, latent_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = True):
        super(ResDenseConv2dLayer_5C, self).__init__()
        self.conv1 = Conv2dLayer(channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv2 = Conv2dLayer(channels + latent_channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv3 = Conv2dLayer(channels + latent_channels * 2, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv4 = Conv2dLayer(channels + latent_channels * 3, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv5 = Conv2dLayer(channels + latent_channels * 4, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation = 'none', sn = sn)
...
class ResinResDenseConv2dLayer_5C(nn.Module): # RRDB
    def __init__(self, channels, latent_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = True):
        super(ResinResDenseConv2dLayer_5C, self).__init__()
        self.conv1 = ResDenseConv2dLayer_5C(channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv2 = ResDenseConv2dLayer_5C(channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
        self.conv3 = ResDenseConv2dLayer_5C(channels, latent_channels, kernel_size, stride, padding, dilation, pad_type, activation, sn)
